...
public class MagistratesPublicListSummaryConverter implements ArtefactSummaryConverter {
    ...
    @Deprecated
    private static void appendAdditionalListInfoV1(StringBuilder output, JsonNode sitting, JsonNode hearing,
                                                   JsonNode hearingCase) {
    ...
    @Deprecated
    public String processMagistratesPublicListV1(JsonNode node) {
        StringBuilder output = new StringBuilder();
        node.get("courtLists").forEach(
            courtList -> courtList.get("courtHouse").get("courtRoom").forEach(
                courtRoom -> courtRoom.get("session").forEach(
